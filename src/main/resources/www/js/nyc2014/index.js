...
(function($){
  ...
  $(function() {
    ...
    $("body").on('click', 'a.toggle', function(e){
      e.preventDefault();
      $(this).parent().parent().parent().find(".preview").slideToggle(100);
      return false;
    });
    ...
    // editing
    $("body").on('submit', "form.propose-edit-form", function (e) {
      e.preventDefault();
      var self = $(this)
      ...
      if (!name.length || !desc.length) {
        ...
        $.post(action, data, function(r) {
          ...
          select.attr('disabled', 'disabled');// this should be disabled by default
          switch(r.status) {
          case 200:
    ...
    $("body").on('click', "a.edit-proposal", function (e) {
      e.preventDefault();
      var self = $(this)
          ...
          case 200:
            ...
            $(lid).append(
              ['<li id="', e.id,'">'
               ...
               , '<a href="#" class="edit-proposal" data-proposal="',e.id,'">make some quick changes</a> or <a href="mailto:doug@meetup.com?subject=please withdraw talk ', e.id,'">Email us</a> if you wish to withdraw this talk.'
               , '</div>'
               , '<div>'
               ...
               , '</select> length talk.'
               , '</div>'
               , '<p class="linkify desc" data-val="',descTxt,'">',descTxt,'</p>'
