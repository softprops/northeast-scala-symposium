...
object Votes {
  ...
  def intent: unfiltered.Cycle.Intent[Any, Any] = {
    case POST(Path(Seg("boston" :: "votes" :: Nil))) &
      CookieToken(ClientToken(token, sec, Some(_), Some(mid))) & Params(p) => Clock("voting for proposal") {
        if(!Meetup.has_rsvp(Meetup.Boston.dayone_event_id, Token(token, sec))) JsonContent ~> ResponseString(
